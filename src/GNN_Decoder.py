...
class GNN_Decoder:
    def __init__(self, params=None):
        ...
        try:
            ...
            print('!!!!!!Input model must be a valid GNN class!!!!!!', flush=True)
        ...
        if p['cuda']:
            ...
            print(f'Moving model to {device}... {"Done" if next(self.model.parameters()).is_cuda else "Impossible"}.')
        ...
        if prefix is None:
            if self.params['save_prefix'] is None:
                print(('No filename was given. '
                       ...
                       'attribute before calling this method.'), flush=True)
                return
    ...
    def save_model(self, filename):
        try:
            ...
            print('No weights found - Model has not yet been trained.', flush=True)
    ...
    def train_with_data_buffer(self,
        ...
        def generate_buffer(buffer_size):
            ...
            factor = 50 if not perfect_stabilizers else 40
            # sample for each error rate:
            if code_size == 5 and repetitions == 9:
            ...
            for sampler in samplers:
                ...
                # del stim_data, observable_flips
                ...
                factor -= 10
            # del stim_data_one_p, observable_flips_one_p
            # print(f'len(stim_data_list)={len(stim_data_list)} ', end='; ')
            ...
            print(f'Sampling ended in {time.time() - start_time}s.', flush=True, end=' ')
            ...
            buffer = [Data(x=torch.tensor(graph[0], device=device), edge_index=torch.tensor(graph[1], device=device),
                           edge_attr=torch.tensor(graph[2], device=device), y=torch.tensor(graph[3], device=device))
                      for sublist in buffer for graph in sublist]
            print(f'Conversion to Data object ended in {time.time() - start_time}s.', flush=True)
            return buffer

        def train_with_buffer(graph_list, shuffle=True):
            '''Trains the network with data from the buffer.'''
            # if repetitions == 9:
            #    print('In train with buffer', flush=True)
            loader = DataLoader(graph_list, batch_size=batch_size, shuffle=shuffle)
            # if repetitions == 9:
            #    print('DataLoader creato', flush=True)
            total_loss = 0.
            ...
            for data in loader:
                # if repetitions == 9:
                #    print("loader iteration", flush=True)
                optimizer.zero_grad()
        ...
        def decode_test_batch(graph_batch):
            ...
            correct_predictions_nontrivial, loss = count_correct_predictions_in_test_batch(graph_batch)
            val_accuracy = correct_predictions_nontrivial / len(graph_batch)

            return val_accuracy, loss / len(graph_batch)
        ...
        def generate_and_decode_test_batch(test_size):
            ...
            print(f'******************************')
            print(f'len(test_data)={len(graph_batch)} (refers to a single error rate)')
            print(f'******************************')
            # Count correct predictions by GNN for nontrivial syndromes
            correct_predictions_nontrivial, _ = count_correct_predictions_in_test_batch(graph_batch)
            ...
            print(f'non-trivial syndromes: {len(graph_batch)}, trivial syndromes: {correct_predictions_trivial}',
                  flush=True)
            print(f'correct, non-trivial predictions: {correct_predictions_nontrivial}', flush=True)
        ...
        if not train:
            test_accuracy = generate_and_decode_test_batch(test_size * buffer_size)
            return test_accuracy
        ...
        ##############################################################################
        if save_to_file:
            print('Will save final results to file after training.', flush=True)

        print(f'Generating data with 4 CPU cores, then moving it to device {device}.', flush=True)
        ...
        if validation:
            ...
            start = time.time()
            if not perfect_stabilizers:
                if code_size == 5 and repetitions in [3, 5, 7]:
                    data_buffer = pd.read_json(f'data/circuit_level_noise/d{code_size}/d{code_size}_dt{repetitions}_training_set.json',
                                               orient='split').to_numpy()
                else:
                    # print('Inizio apertura.', flush=True)
                    # start_prova = time.time()
                    data_buffer = pd.read_json(f'data/circuit_level_noise/d{code_size}/d{code_size}_dt{repetitions}_training_set.json',
                                               orient='records', lines=True, chunksize=10**4)
                    # print(f'Aperto in {time.time() - start_prova}s.\nInizio composizione', flush=True)
                    # start_prova = time.time()
                    data_buffer = pd.concat(data_buffer, ignore_index=True).to_numpy()
                    # print(data_buffer, flush=True)
                    # print(f'Composto in {time.time() - start_prova}s.', flush=True)
                gc.collect()
                # print(data_buffer, flush=True)
                data_buffer = [Data(x=torch.tensor(e[0], device=device),
                                    edge_index=torch.tensor(e[1], device=device, dtype=torch.int64),
                                    edge_attr=torch.tensor(e[2], device=device),
                                    y=torch.tensor(e[3], device=device)) for e in data_buffer]
                # print(f'len={len(data_buffer)}\nfirst={data_buffer[0]}', flush=True)
                # print('leggo validation', flush=True)
                if code_size == 5 and repetitions in [3, 5, 7]:
                    test_val_batch = pd.read_json(
                        f'data/circuit_level_noise/d{code_size}/d{code_size}_dt{repetitions}_validation_set.json',
                        orient='split').to_numpy()
                else:
                    test_val_batch = pd.read_json(
                        f'data/circuit_level_noise/d{code_size}/d{code_size}_dt{repetitions}_validation_set.json',
                        orient='records', lines=True, chunksize=10**3)
                    test_val_batch = pd.concat(test_val_batch, ignore_index=True).to_numpy()
                # print(test_val_batch, flush=True)
                test_val_batch = [Data(x=torch.tensor(e[0], device=device),
                                       edge_index=torch.tensor(e[1], device=device, dtype=torch.int64),
                                       edge_attr=torch.tensor(e[2], device=device),
                                       y=torch.tensor(e[3], device=device)) for e in test_val_batch]
                # print(f'len={len(test_val_batch)}\nfirst={test_val_batch[0]}', flush=True)
            else:
                data_buffer = pd.read_json(
                    f'data/perfect_stabilizers/d{code_size}/d{code_size}_dt{repetitions}_ps_training_set.json',
                    orient='records', lines=True, chunksize=10**4)
                data_buffer = pd.concat(data_buffer, ignore_index=True).to_numpy()
                gc.collect()
                # print(data_buffer, flush=True)
                data_buffer = [Data(x=torch.tensor(e[0], device=device),
                                    ...
                                    y=torch.tensor(e[3], device=device)) for e in data_buffer]
                test_val_batch = pd.read_json(
                    f'data/perfect_stabilizers/d{code_size}/d{code_size}_dt{repetitions}_ps_validation_set.json',
                    orient='records', lines=True, chunksize=10**3)
                test_val_batch = pd.concat(test_val_batch, ignore_index=True).to_numpy()
                test_val_batch = [Data(x=torch.tensor(e[0], device=device),
                                       edge_index=torch.tensor(e[1], device=device, dtype=torch.int64),
                                       edge_attr=torch.tensor(e[2], device=device),
                                       y=torch.tensor(e[3], device=device)) for e in test_val_batch]
            print(f'Data correctly red. Elapsed time: {time.time() - start}s', flush=True)
            gc.collect()

            print(f'******************************', flush=True)
            print(f'len(training_data)={len(data_buffer)} ; len(validation_data)={len(test_val_batch)}', flush=True)
            print(f'******************************', flush=True)
            gc.collect()
            try:
        ...
        start = len(self.continuous_training_history['accuracy'])

        for ix in range(start, num_iterations + start):
            if benchmark:
                fit_start = time.perf_counter()
            if ix >= 250:
                self.set_learning_rate(0.00001)
            ...
            if replacements_per_iteration > 0:
                del data_buffer[: (replacements_per_iteration * batch_size * len(error_rate))]
                # create new data:
                data_buffer.extend(generate_buffer(replacements_per_iteration))
            gc.collect()
            ...
            if validation:
                val_acc, val_loss = decode_test_batch(test_val_batch)
                self.continuous_training_history['val_acc'].append(val_acc)
                self.continuous_training_history['val_loss'].append(val_loss)
            ...
            if (num_iterations <= 100) or ((ix + 1) % training_print_spacing == 0) or (ix == 0):
                ...
                if validation:
                    training_print_string += f'Validation Acc: {val_acc:.4f}\tValidation Loss: {val_loss:.4f}\t'
                training_print_string += (f'Cumulative # of training samples: '
                                          f'{(previously_completed_samples + sample_count * (ix + 1)):.2e}\t')
                print(f'{training_print_string}.', flush=True, end=' ')
            print('Saving model...', flush=True, end=' ')
            # format: d5_d_t_5_rep_id2020567_epoch50.pt means that 50 epochs ended, i.e. 250 remaining
            '''
            file_name = os.path.join(self.params['save_path'], f'{self.params["save_prefix"]}_epoch{ix}')
            if os.path.exists(file_name):
                os.remove(file_name)
            '''
            file_name = f'{self.params["save_prefix"]}_epoch{ix + 1}'
            self.save_attributes_to_file(prefix=file_name, suffix='')
            print(f'Model successfully saved in {file_name}. Total time for epoch: {time.time() - start_epoch}s.', flush=True)

        print('Completed all training iterations!', flush=True)
        if save_to_file:
            print('Saving final model, history and data to file.', flush=True)
            self.save_attributes_to_file(
                prefix=save_file_prefix,
                suffix='')
            if replacements_per_iteration > 0:
                data_buffer = [{key: val.tolist() for key, val in elem.to_dict().items()} for elem in data_buffer]
                gc.collect()
                data_buffer = pd.DataFrame(data_buffer)
                gc.collect()
                if not perfect_stabilizers:
                    if code_size == 5 and repetitions in [3, 5, 7]:
                        data_buffer.to_json(
                            f'data/circuit_level_noise/d{code_size}/d{code_size}_dt{repetitions}_training_set.json',
        ...
        if benchmark:
            ...
            print(f'Training setup: {time_setup}', flush=True)
            print(f'Sampling and Graphing: {time_sample}', flush=True)
            print(f'Fitting: {time_fit}', flush=True)
            print(f'\tSum: {time_setup + time_sample + time_fit}', flush=True)
...
def generate_batch(stim_data_list,
    ...
    for i in range(len(stim_data_list)):
        ...
        batch.append(graph)
        del graph, true_eq_class, syndrome
    return batch
